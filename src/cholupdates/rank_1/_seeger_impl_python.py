...
def downdate(L: np.ndarray, v: np.ndarray) -> None:
    ...
    blas_trsv, blas_dot, blas_rotg, blas_rot = scipy.linalg.get_blas_funcs(
        ("trsv", "dot", "rotg", "rot"), (L, v)
    )
    ...
    if L_aug_cols_1n.flags.f_contiguous:
        for k in range(N - 1, -1, -1):
            ...
            # Note: The following two operations will be performed by a single call to
            # `rotg` in C/Fortran. However, Python can not modify `Float` arguments.
            c, s = blas_rotg(q_np1, q_1n[k])
            q_np1, q_1n[k] = blas_rot(q_np1, q_1n[k], c, s)
